# v27.8i: 3-WEEK EXPOSURE FORECAST EXPERIMENT
# Only forecast horizon defaults changed from 20 to 3; architecture/losses otherwise unchanged.
...
# Purpose: stabilize point exposure forecasts and learn joint 3-week exposure regimes.
...
def prepare_data_from_sample(
    ...
    # Sort first so the same seed always selects the same ASINs across snapshots.
    unique_asins = np.sort(df["asin"].dropna().unique())

    # n_asins=None (or <=0, or >= available count) means use every ASIN.
    if n_asins is None or int(n_asins) <= 0 or int(n_asins) >= len(unique_asins):
        ...
        sample_asins = rng.choice(
            unique_asins,
            size=min(int(n_asins), len(unique_asins)),
            replace=False,
        )
        selection_note = "SAMPLED"

    out = df[df["asin"].isin(set(sample_asins))].copy()
    print(
        f"ASIN selection={selection_note} | "
        f"ASINs: {len(sample_asins):,}/{len(unique_asins):,} | Rows: {len(out):,}"
    )
    return out
...
class ExposureDataset(Dataset):
    def __init__(self, data, history=13, horizon=3, mode="train",
...
class TCNDecoderWithCrossAttn(nn.Module):
    ...
    """
    def __init__(self, d_model, context_dim, horizon=3,
        ...
        # Auxiliary occurrence head. With ENN, z controls the 3-week active/zero regime.
...
class ExposureForecastModelV2(nn.Module):
    ...
    def __init__(self, input_dim, context_dim,
                 d_model=64, horizon=3, n_heads=4, dropout=0.10,
...
def print_exposure_diagnostics(pred_df):
    ...
    Kept:
      ...
      4) ASIN-level 3-week sum diagnostics
    ...
    # ── ASIN级别诊断 ─────────────────────────────────────────
    print("\n" + "=" * 100)
    print(f"ASIN-LEVEL {int(by_h["horizon"].max())}-WEEK SUM")
    print("=" * 100)
...
def diagnose_encoder_decoder_performance(model, va_ld, pred_df=None, max_batches=None, device=None):
    ...
    Encoder checks:
      ...
      - Can h_last predict future 3-week magnitude?
...
def run_exposure_v2_final_scot_5000(
    ...
    Final single-window setup:
      - sample n_asins ASINs (use None for all ASINs)
      - optionally intersect with SCOT ASINs
      - train on sliding windows before the final holdout
      - validate/predict the latest 3-week window only
...
def diagnose_promo_adjusted_rank(pred_df, target="in_stock"):
    ...
    Two views are printed:
      ...
      2) ASIN-level 3-week WAPE: since AMXL WAPE is often interpreted per item/path,
         aggregate each ASIN's final 3-week exposure and inspect whether errors concentrate
    ...
    print(f"\nASIN-LEVEL {int(pred_df['horizon'].max())}-WEEK WAPE BY AVG PROMO-ADJUSTED RANK")
...
# ============================================================
# S3 SNAPSHOT ROLLING RUNNER — NO SCOT JOIN
#
# S3 location is fixed from the user's snapshot folder:
#   bucket: amxl-asin-forecast590184089576
#   prefix: amxl-asin-forecast-intern/data_for_model/df_head_body_add_holiday_
#
# Each dated ETLM snapshot is processed independently in chronological order.
# Raw snapshots are never concatenated. Only generated exposure hats are combined.
# ============================================================

S3_EXPOSURE_SNAPSHOT_BUCKET = "amxl-asin-forecast590184089576"
S3_EXPOSURE_SNAPSHOT_PREFIX = (
    "amxl-asin-forecast-intern/data_for_model/df_head_body_add_holiday_"
)
DEFAULT_FULL_HAT_CSV = "exposure_hat_for_demand full.csv"


def list_exposure_snapshot_files_s3(
    s3_bucket=S3_EXPOSURE_SNAPSHOT_BUCKET,
    s3_prefix=S3_EXPOSURE_SNAPSHOT_PREFIX,
    filename_regex=r"df_head_body_add_holiday_(\d{4})-(\d{2})-(\d{2})_?ETLM_[vV]3\.csv$",
    start_date=None,
    end_date=None,
    s3_client=None,
):
    """List the dated ETLM snapshots and sort them by cut date."""
    try:
        import boto3
    except ImportError as exc:
        raise ImportError("boto3 is required for the S3 rolling runner.") from exc
    ...
    for page in paginator.paginate(Bucket=s3_bucket, Prefix=s3_prefix):
        for obj in page.get("Contents", []):
            ...
            cut_date = pd.Timestamp("-".join(match.groups()))
            if start_ts is not None and cut_date < start_ts:
                continue
            if end_ts is not None and cut_date > end_ts:
                continue
    ...
    if files.empty:
        raise FileNotFoundError(
            f"No snapshot files matched under s3://{s3_bucket}/{s3_prefix}"
        )

    files = files.sort_values(["cut_date", "s3_key"]).reset_index(drop=True)
    print(f"Found {len(files)} snapshot files")
    print(
        f"Date range: {files['cut_date'].min().date()} -> "
        f"{files['cut_date'].max().date()}"
    )
    return files


def read_snapshot_csv_s3(
    s3_bucket,
    s3_key,
    s3_client=None,
    **read_csv_kwargs,
):
    """Read one snapshot CSV directly through boto3; s3fs is not required."""
    try:
        import boto3
    except ImportError as exc:
        raise ImportError("boto3 is required for the S3 rolling runner.") from exc
    ...
    body = obj["Body"].read()
    df = pd.read_csv(BytesIO(body), **read_csv_kwargs)

    required = {"asin", "order_week"}
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f"Snapshot {s3_key} is missing columns: {sorted(missing)}")
...
def run_exposure_s3_snapshot_rolling(
    # First smoke test defaults: 5000 ASINs, two earliest snapshots, three epochs.
    n_asins=None,
    max_snapshots=None,
    select_latest=False,
    snapshot_stride=1,
    ...
    """
    Run Exposure independently for each chronological S3 snapshot.

    Output layout:
      exposure_full_h3/
        per_cut/exposure_hat_cut_YYYY-MM-DD.csv
        exposure_hat_for_demand_full_h3.csv
        exposure_rolling_summary_h3.csv
        exposure_rolling_errors_h3.csv

    Important behavior:
      1. No SCOT intersection or SCOT join is performed.
      2. Snapshots are processed in chronological order.
      3. Each successful cut is saved immediately, so an interrupted run can resume.
      4. Historical cut files are never overwritten unless resume_existing=False.
      5. Use n_asins=None and max_snapshots=None for all ASINs and all snapshots.
    """
    ...
    full_hat_csv = output_root / "exposure_hat_for_demand_full_h3.csv"
    summary_csv = output_root / "exposure_rolling_summary_h3.csv"
    errors_csv = output_root / "exposure_rolling_errors_h3.csv"

    files = list_exposure_snapshot_files_s3(
        s3_bucket=s3_bucket,
        s3_prefix=s3_prefix,
    ...
    stride = max(int(snapshot_stride), 1)
    files = files.iloc[::stride].reset_index(drop=True)

    if max_snapshots is not None:
        n = max(int(max_snapshots), 0)
        files = files.tail(n) if select_latest else files.head(n)
    ...
    for i, row in files.iterrows():
        cut_date = pd.Timestamp(row["cut_date"])
        cut_str = cut_date.strftime("%Y-%m-%d")
        cut_hat_path = per_cut_dir / f"exposure_hat_cut_{cut_str}.csv"

        print("\n" + "#" * 110)
        print(f"SNAPSHOT {i + 1}/{len(files)} | cut={cut_str}")
        print(row["s3_uri"])
        print(f"Output: {cut_hat_path}")
        print("#" * 110)

        if resume_existing and cut_hat_path.exists():
            try:
                existing = pd.read_csv(cut_hat_path, nrows=5)
                required = {"forecast_cut", "asin"}
                if required.issubset(existing.columns):
                    n_rows = sum(1 for _ in open(cut_hat_path, "rb")) - 1
                    cut_hat_paths.append(cut_hat_path)
                    summary_rows.append({
                        "forecast_cut": cut_date,
                        "status": "skipped_existing",
                        "snapshot_s3_key": row["s3_key"],
                        "input_rows": np.nan,
                        "input_asins": np.nan,
                        "model_asins": np.nan,
                        "hat_rows": max(n_rows, 0),
                        "hat_csv": str(cut_hat_path),
                        "error": "",
                    })
                    print("Existing valid cut file found; skipping training.")
                    continue
            except Exception as exc:
                print(f"Existing file could not be validated; retraining: {exc}")

        try:
            data_cut = read_snapshot_csv_s3(
                s3_bucket=s3_bucket,
                s3_key=row["s3_key"],
                s3_client=s3_client,
            )
            print(
                f"Loaded rows={len(data_cut):,} | "
                f"ASINs={data_cut['asin'].nunique():,} | "
                f"weeks={data_cut['order_week'].nunique():,} | "
                f"max_week={data_cut['order_week'].max()}"
            )
            ...
            hat = result["exposure_hat_for_demand"].copy()
            hat["forecast_cut"] = cut_date
            hat["snapshot_s3_key"] = row["s3_key"]

            sort_cols = [
                c for c in ["forecast_cut", "asin", "order_week", "horizon"]
                if c in hat.columns
            ]
            if sort_cols:
                hat = hat.sort_values(sort_cols).reset_index(drop=True)

            # Save immediately after each cut. This protects completed work if a later cut fails.
            hat.to_csv(cut_hat_path, index=False)
            cut_hat_paths.append(cut_hat_path)
            ...
            summary_rows.append({
                ...
                "status": "ok",
                "snapshot_s3_key": row["s3_key"],
                "input_rows": len(data_cut),
                "input_asins": data_cut["asin"].nunique(),
                "model_asins": result["source_df"]["asin"].nunique(),
                "hat_rows": len(hat),
                "hat_csv": str(cut_hat_path),
                "error": "",
            })

            # Persist progress summary after every completed cut.
            pd.DataFrame(summary_rows).sort_values("forecast_cut").to_csv(summary_csv, index=False)

            del result, data_cut, hat
            if torch.cuda.is_available():
                torch.cuda.empty_cache()

        except Exception as exc:
            print(f"FAILED cut={cut_str}: {type(exc).__name__}: {exc}")
            summary_rows.append({
                "forecast_cut": cut_date,
                "status": "failed",
                "snapshot_s3_key": row["s3_key"],
    ...
    summary = pd.DataFrame(summary_rows).sort_values("forecast_cut").reset_index(drop=True)
    summary.to_csv(summary_csv, index=False)

    errors = summary[summary["status"] == "failed"].copy()
    errors.to_csv(errors_csv, index=False)

    valid_paths = [Path(p) for p in cut_hat_paths if Path(p).exists()]
    if not valid_paths:
        raise RuntimeError("No snapshot completed successfully; no full hat CSV was created.")

    # Combine all per-cut outputs only after processing finishes.
    # The per-cut files remain the source of truth for restart and demand-by-cut reads.
    combined_parts = []
    for path in sorted(valid_paths):
        part = pd.read_csv(path, parse_dates=["forecast_cut"])
        combined_parts.append(part)

    combined_hats = pd.concat(combined_parts, ignore_index=True)
    sort_cols = [
        c for c in ["forecast_cut", "asin", "order_week", "horizon"]
        if c in combined_hats.columns
    ]
    if sort_cols:
        combined_hats = combined_hats.sort_values(sort_cols).reset_index(drop=True)

    combined_hats.to_csv(full_hat_csv, index=False)

    print("\nROLLING SNAPSHOT SUMMARY")
    print(summary[["forecast_cut", "status", "model_asins", "hat_rows", "error"]])
    print(f"\nPer-cut folder: {per_cut_dir}")
    print(f"Saved combined demand hat CSV: {full_hat_csv}")
    print(
        f"Rows={len(combined_hats):,} | "
        f"forecast_cuts={combined_hats['forecast_cut'].nunique():,} | "
        f"ASINs={combined_hats['asin'].nunique():,}"
    )
    ...
    return {
        ...
        "summary": summary,
        "errors": errors,
        "exposure_hat_for_demand": combined_hats,
        "output_root": str(output_root),
        "per_cut_dir": str(per_cut_dir),
        "full_hat_csv": str(full_hat_csv),
...
# ============================================================
# FINAL USAGE — H3 rolling, no SCOT join
# ============================================================
# S3 bucket and prefix are already configured in this file.
# Results are written to ./exposure_full_h3/ and do not overwrite 20-week outputs.
#
# Smoke test: 5000 ASINs, two earliest snapshots, three epochs.
#
# %run -i tcn_exposure_v2_v27_8l_S3_ROLLING_H3_FULL_ASIN_DIAGNOSTICS_FIXED.py
#
# rolling_result_h3 = run_exposure_s3_snapshot_rolling(
#     n_asins=5000,
#     max_snapshots=2,
#     select_latest=False,
#     history=13,
...
# )
#
# Full run: all ASINs and all snapshots.
#
# rolling_result_h3 = run_exposure_s3_snapshot_rolling(
#     n_asins=None,
#     max_snapshots=None,
