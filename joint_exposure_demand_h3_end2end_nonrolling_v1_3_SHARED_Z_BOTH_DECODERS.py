...
class ExposureDecoderEpinet(nn.Module):
    """z-conditioned exposure residual with stop-gradient base features.

    The deterministic exposure decoder learns the marginal exposure forecast.
    This epinet adds one shared-z residual to:
      1) total exposure raw score
      2) buy-box ratio logit
      3) in-stock ratio logit

    The same sampled z is then passed to the demand branch, so each z represents
    one coherent joint exposure-demand H1-H3 scenario.
    """
    def __init__(
        self,
        d_decoder,
    ...
    def forward(self, decoder_hidden, future_context, z):
        B, H, _ = decoder_hidden.shape
        z_h = z[:, None, :].expand(-1, H, -1)

        # Match the ENN stop-gradient design used in the demand branch.
        inp = torch.cat(
            [
        ...
        learnable = self.learnable(inp)
        prior = self.prior(inp) * self.prior_scale
        return self.residual_scale * (learnable + prior)


class JointExposureDemandH3(nn.Module):
    """Joint H3 model where one shared epistemic index affects both tasks.

    Architecture
    ------------
    shared history encoder
        -> deterministic exposure decoder
        -> z-conditioned exposure epinet residual
        -> exposure_hat(z)
        -> demand decoder conditioned on exposure_hat(z)
        -> demand epinet residual using the same z

    The exposure and demand branches therefore produce a coherent joint
    H1-H3 scenario for every sampled z.
    """
    ...
    def __init__(
        ...
        if context_dim < 3:
            raise ValueError(
                "future_context must reserve its final 3 columns for exposure hats."
            )
        ...
        # Shared deterministic base encoder.
        self.encoder = TCNSparseAttnEncoder(input_dim, d_model, horizon)
        ...
        # z is generated before exposure prediction from shared history features
        # and future-known base context only. No predicted or true future DPH is
        # used to generate z.
        ...
        # z-conditioned exposure residual.
        self.exposure_epinet = ExposureDecoderEpinet(
            d_decoder=d_model,
            context_dim=self.base_context_dim,
            d_z=d_z,
            hidden=64,
            prior_scale=0.15,
            residual_scale=exposure_z_scale,
        )
    ...
    def _base_future_context(self, future_context):
        # Final 3 slots are reserved for internally predicted exposure hats.
        return future_context[:, :, :self.base_context_dim]

    def _exposure_base(self, H_enc, b_t, peak_score, future_context):
        ...
        exp_features = torch.cat([dec_h, fc_base], dim=-1)

        total_raw = self.exp_total_head(exp_features).squeeze(-1)
        ratio_logits = self.exp_ratio_head(exp_features)
        active_logits = self.exp_active_head(exp_features)
    ...
    def _exposure_one_z(self, exposure_base, z):
        residual = self.exposure_epinet(
            exposure_base["dec_h"],
            exposure_base["fc_base"],
            z,
        )

        total = F.softplus(
            exposure_base["total_raw"] + residual[..., 0]
        )
        ratio_logits = exposure_base["ratio_logits"] + residual[..., 1:3]
        ratios = torch.sigmoid(ratio_logits)

        buy = total * ratios[..., 0]
        instock = total * ratios[..., 1]

        return torch.stack([total, buy, instock], dim=-1)
    ...
    def _demand_one_z(
        ...
        demand_context = self._demand_context(
            future_context,
            exposure_hat,
        )
        ...
        demand_dec_in = torch.cat(
            [demand_dec_h, demand_context],
            dim=-1,
        )
        peak_mu = F.softplus(
            self.peak_mu_head(demand_dec_in).squeeze(-1)
        )
        peak_gate = torch.sigmoid(
            self.peak_gate_head(demand_dec_in).squeeze(-1)
        )

        mu_e, al_e = self.epinet(phi, z)
        dec_mu_e, dec_al_e = self.decoder_epinet(
            demand_dec_h,
            demand_context,
            z,
        )
        ...
        return mu, alpha, demand_context
    ...
    def forward(self, x, future_context, nZ=8):
        ...
        # Stop-gradient feature into the z generator, consistent with ENN.
        phi = h_t.detach()
        ...
        for _ in range(nZ):
            ...
            # The same z affects exposure and demand.
            exposure_hat_z = self._exposure_one_z(
                exposure_base,
                z,
            )
            mu, alpha, demand_context_z = self._demand_one_z(
                H_enc,
                b_t,
                peak_score,
                h_t,
            ...
            exposure_hats.append(exposure_hat_z)
            demand_preds.append((mu, alpha))
            demand_contexts.append(demand_context_z)

        exposure_stack = torch.stack(exposure_hats, dim=1)
        exposure_hat_mean = exposure_stack.mean(dim=1)

        return {
            "demand_preds": demand_preds,
            "z_reg": z_reg,
            "exposure_hats": exposure_hats,
            "exposure_hat": exposure_hat_mean,
            "exposure_active_logits": exposure_base["active_logits"],
            "demand_contexts": demand_contexts,
            "z_mean": z_mean,
            "z_std": z_std,
        }
    ...
    def predict(self, x, future_context, M=100):
        ...
        with torch.no_grad():
            ...
            for mu, alpha in out["demand_preds"]:
                ...
                demand_samples.append(dist.sample().float())

            demand_samples = torch.stack(
                demand_samples,
                dim=1,
            )
            p50 = demand_samples.quantile(0.5, dim=1)
            p70 = torch.maximum(
                demand_samples.quantile(0.7, dim=1),
                p50,
            )

            # Use the mean across epistemic exposure scenarios as the point hat.
            exposure_hat = out["exposure_hat"]

        return p50, p70, exposure_hat
...
def train_joint_exposure_demand_h3(
    ...
    beta_tail=0.5, lambda_under=0.15, lambda_z_reg=1.0,
    ...
    for epoch in range(epochs):
        ...
        for b in tr_ld:
            ...
            demand_loss = demand_nll + lambda_under * under + lambda_z_reg * out["z_reg"]

            exp_loss = sum(
                joint_exposure_loss(
                    exp_hat_z,
                    out["exposure_active_logits"],
                    true_exp,
                )
                for exp_hat_z in out["exposure_hats"]
            ) / max(1, len(out["exposure_hats"]))
...
def run_joint_exposure_demand_h3_end2end(
    ...
    remove_extreme=True,
    extreme_q=0.99,
    output_csv="joint_exposure_demand_h3_shared_z_both_decoders.csv",
    remove_oos_dp=True,
):
    """Run non-rolling H3 joint model on one snapshot.

    V1.3 keeps the two-step cohort/WAPE logic, but uses one shared epistemic
    index z for both the exposure and demand branches. No exposure CSV is read.
    """
    if horizon != 3:
        raise ValueError("Use horizon=3 for this file.")

    print("=" * 88)
    print("JOINT EXPOSURE + DEMAND H3 END-TO-END V1.3 | SHARED-Z BOTH DECODERS")
    ...
    print(f"Shared z affects exposure and demand: YES | exposure_z_scale={exposure_z_scale:.3f}")
    print("=" * 88)

    raw = data_raw1.copy()
    # Reserve the last three future-context columns without reading an external CSV.
    for c in ["attn_pred_total_dph", "attn_pred_buy_box_dph", "attn_pred_instock_dph"]:
        raw[c] = 0.0
    ...
    if scot_df is not None:
        ...
        data_small_raw = prepare_data_sample(raw, n_asins)
        sample_asin_df = pd.DataFrame({"asin": sorted(data_small_raw["asin"].astype(str).unique())})
    ...
    model = JointExposureDemandH3(
        ...
        exposure_z_scale=exposure_z_scale,
    )
    ...
    demand_wape_p50 = np.abs(forecast_df["p50_amxl"] - forecast_df["fbi_demand"]).sum() / (
        np.abs(forecast_df["fbi_demand"]).sum() + 1e-8
    )
    demand_wape_p70 = np.abs(forecast_df["p70_amxl"] - forecast_df["fbi_demand"]).sum() / (
        np.abs(forecast_df["fbi_demand"]).sum() + 1e-8
    )

    print("\nJOINT EXPOSURE METRICS")
    print(exposure_metrics.round(4).to_string(index=False))
    print(f"\nDemand WAPE | P50={demand_wape_p50:.4f} | P70={demand_wape_p70:.4f}")
    ...
    result = {
        ...
        "demand_wape_p50": demand_wape_p50,
        "demand_wape_p70": demand_wape_p70,
        "tr_ld": tr_ld,
        "va_ld": va_ld,
...
# ============================================================================
# RECOMMENDED USAGE (NON-ROLLING H3)
# ============================================================================
