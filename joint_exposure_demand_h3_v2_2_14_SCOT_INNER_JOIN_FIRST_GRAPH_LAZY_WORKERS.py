...
def prepare_data_from_sample_scot_intersection(
    ...
    # before we know which rows are eligible.
    scot_asins = (
        scot_df["asin"]
        .dropna()
        .astype(str)
    ...
    eligible = data_raw1.loc[eligible_mask].copy()
    eligible["asin"] = eligible["asin"].astype(str).str.strip()
    eligible["order_week"] = pd.to_datetime(eligible["order_week"], errors="coerce")
    ...
    if n_asins is None:
        selected_asins = eligible_asins
    else:
        rng = np.random.default_rng(seed)
        selected_asins = rng.choice(
            eligible_asins,
            size=min(n_asins, len(eligible_asins)),
            replace=False,
        )

    selected_set = set(selected_asins)
    if n_asins is None or len(selected_set) == len(eligible_asins):
        data_small = eligible
    else:
        t_sample = time.time()
        data_small = eligible.loc[eligible["asin"].isin(selected_set)].copy()
        del eligible
        gc.collect()
        print(
    ...
    sample_asin_df = pd.DataFrame({"asin": list(selected_asins)})
...
class DemandDataset(_GraphContextMixin, _ORIGINAL_DEMAND_DATASET_BEFORE_GRAPH_CONTEXT):
    ...
    The original implementation materialized every sliding-window sample and
    every tensor inside ``__init__``.  This version stores only ``(asin, start)``
    indices and constructs a sample in ``__getitem__``.  Window boundaries,
    DPH proxy formulas, graph-context injection, targets, and dtypes are kept
    unchanged.
    """
    ...
    def __init__(self, data, history=52, horizon=3, mode="train", val_weeks=20,
        ...
        self.mode = str(mode)
        self.sample_index = []
        self._worker_first_sample_logged = set()

        print(
            f"[STAGE] DemandDataset LAZY START | mode={self.mode} | "
            f"ASINs={len(data):,} | history={self.history} | horizon={self.horizon}",
            flush=True,
        )

        graph_t0 = time.perf_counter()
        print(f"[DATASET-LAZY] graph neighbor initialization START | mode={self.mode}", flush=True)
        self._init_graph_context(min_graph_neighbors=min_graph_neighbors)
        print(
            f"[DATASET-LAZY] graph neighbor initialization DONE | mode={self.mode} | "
            f"elapsed={time.perf_counter()-graph_t0:.2f}s",
            flush=True,
        )

        # ``num_build_workers`` is retained for API compatibility.  Lazy
        # construction is parallelized by DataLoader workers instead.
        if num_build_workers not in (None, 0, 1):
            print(
                f"[DATASET-LAZY] num_build_workers={num_build_workers} ignored during __init__; "
                "sample construction will use DataLoader num_workers.",
                flush=True,
            )
        ...
        eligible_asins = 0
        total_windows = 0
        report_every = max(250, min(1000, total_asins // 10 if total_asins else 250))
        ...
        for asin_i, (asin, d) in enumerate(data.items(), start=1):
            ...
            if self.mode == "train":
                n_starts = max(
                    0,
                    T - self.val_weeks - self.horizon - self.history + 1,
                )
                if n_starts > 0:
                    eligible_asins += 1
                    self.sample_index.extend((asin, start) for start in range(n_starts))
                    total_windows += n_starts
            else:
                start = T - self.history - self.horizon
                if start >= 0:
                    eligible_asins += 1
                    self.sample_index.append((asin, start))
                    total_windows += 1

            if asin_i % report_every == 0 or asin_i == total_asins:
                elapsed = time.perf_counter() - index_t0
                rate = asin_i / elapsed if elapsed > 0 else 0.0
                remain = total_asins - asin_i
                eta = remain / rate if rate > 0 else 0.0
                print(
                    f"[DATASET-LAZY] index progress | mode={self.mode} | "
                    f"ASINs={asin_i:,}/{total_asins:,} | eligible={eligible_asins:,} | "
                    f"samples={total_windows:,} | elapsed={elapsed:.2f}s | ETA={eta:.2f}s",
                    flush=True,
                )

        print(
            f"[STAGE] DemandDataset LAZY DONE | mode={self.mode} | "
            f"eligible_asins={eligible_asins:,} | samples={len(self.sample_index):,} | "
            f"index_elapsed={time.perf_counter()-index_t0:.2f}s | "
            f"total_elapsed={time.perf_counter()-_ds_t0:.2f}s",
            flush=True,
        )
    ...
    def __getitem__(self, i):
        asin, start = self.sample_index[i]
        d = self.data[asin]
        ...
        # Preserve the original DPH proxy and graph-context logic exactly.
        ...
        # Log only the first sample created by each process/worker.
        worker_info = torch.utils.data.get_worker_info()
        worker_id = worker_info.id if worker_info is not None else "main"
        if worker_id not in self._worker_first_sample_logged:
            self._worker_first_sample_logged.add(worker_id)
            print(
                f"[DATASET-LAZY] first sample built | mode={self.mode} | "
                f"worker={worker_id} | asin={asin} | start={start} | "
                f"x_shape={np.asarray(d['features'][start:history_end]).shape} | "
                f"fc_shape={np.asarray(fc).shape}",
                flush=True,
            )

        # np.asarray(dtype=float32) + torch.from_numpy preserves the values and
        # dtype used by the former torch.tensor(..., dtype=torch.float32) path,
        # while avoiding an unnecessary extra copy when the slice is already
...
def run_joint_exposure_demand_h3_end2end(
    ...
    # Inner-join the raw cohort with SCOT before adding columns or running any
    # expensive preprocessing. This avoids copying/processing ASINs that cannot
    # participate in the final SCOT comparison.
    data_small_raw, sample_asin_df, intersect_asin_df = prepare_data_from_sample_scot_intersection(
        data_raw1=data_raw1, scot_df=scot_df, n_asins=n_asins, seed=seed
    )

    # Reserve the final three future-context columns only on the reduced cohort.
    # The model predicts these exposure hats internally.
    for c in ["attn_pred_total_log", "attn_pred_buy_box_log", "attn_pred_instock_log"]:
        data_small_raw[c] = 0.0
