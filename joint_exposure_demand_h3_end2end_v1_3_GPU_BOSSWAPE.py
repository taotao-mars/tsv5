...
print(f"[joint_h3] using device: {DEVICE}")
...
# of any locally rolled abs(pred-true)/true formula.
# =====================================================
...
def weekly_error_check(df, cols, cols_type):
    ...
    cols_type: 'p50' or 'p70', selects which quantile's per-horizon breakdown to build
    """
    if df.shape[0] > 0:
...
class JointExposureDemandH3(nn.Module):
    ...
    def predict(self, x, future_context, M=100):
        ...
        with torch.no_grad():
            ...
            return p50, p70, out["exposure_hat"]
...
def _joint_exposure_metrics_by_horizon(df, near_zero_thresh=0.5):
    """Exposure-hat quality check, split by H1/H2/H3 (fcst_week_index).

    In addition to WAPE/ratio/corr per horizon, this flags two failure
    modes that a single overall WAPE number can hide:
      - zero_rate_true vs near_zero_rate_pred: whether the hat is
        collapsing to near-zero at roughly the true rate of zero weeks
        (a proxy for whether the active/occurrence head is doing anything).
      - gross_over_rate / gross_under_rate: share of rows where the hat is
        more than 2x too high / less than half the true value, which a
        good mean/corr can still hide if errors cancel out.
    """
    out = []
    for name, true_col, pred_col in [
        ("total", "true_future_total_dph", "pred_total_dph_hat"),
        ...
        for h, g in df.groupby("fcst_week_index"):
            y = g[true_col].to_numpy(float)
            p = g[pred_col].to_numpy(float)
            denom = np.abs(y).sum() + 1e-8
            active = y > 0
            gross_over = ((p > 2 * y) & active).mean() if active.any() else np.nan
            gross_under = ((p < 0.5 * y) & active).mean() if active.any() else np.nan
            out.append({
                "channel": name,
                "fcst_week_index": h,
                "wape": np.abs(p - y).sum() / denom,
                ...
                "near_zero_rate_pred": (p < near_zero_thresh).mean(),
                "gross_over_rate": gross_over,
                "gross_under_rate": gross_under,
            })
    return pd.DataFrame(out).sort_values(["channel", "fcst_week_index"]).reset_index(drop=True)
...
def run_joint_exposure_demand_h3_end2end(
    ...
    if scot_df is None:
        raise ValueError(
            "scot_df is required: WAPE is now reported exclusively via the "
    ...
    exposure_metrics_by_horizon = _joint_exposure_metrics_by_horizon(forecast_df)

    print("\nJOINT EXPOSURE METRICS (overall)")
    print(exposure_metrics.round(4).to_string(index=False))
    print("\nJOINT EXPOSURE METRICS (by horizon)")
    print(exposure_metrics_by_horizon.round(4).to_string(index=False))
    ...
    result = {
        ...
        "exposure_metrics_by_horizon": exposure_metrics_by_horizon,
        "tr_ld": tr_ld,
        "va_ld": va_ld,
