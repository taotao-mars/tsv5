...
class TCN_ENN(nn.Module):
    """
    Demand v12d: external predicted exposure-3 hats + exposure-conditioned demand gate.

    Main idea:
      if predicted future exposure is near zero, demand should be near zero;
      but the gate is soft and active-protected so recent demand activity / peak evidence
      can prevent severe underforecast when exposure hats are imperfect.
    ...
    def __init__(self, input_dim=34, context_dim=2, d_model=32,
                 ...
                 exposure_gate_threshold=0.35,
                 exposure_gate_temperature=0.15,
                 exposure_gate_min=0.05,
                 exposure_gate_recent_active_floor=0.30,
                 exposure_gate_peak_floor=0.20):
        super().__init__()
        self.d_z = d_z
        self.horizon = horizon
        ...
        # v12d exposure-conditioned demand gate.
        # The external exposure hats are log1p(counts) in the last three future_context cols:
        #   total, buy_box, in_stock. Demand is impossible/rare when exposure is near zero,
        #   so this gate softly suppresses mu in low-exposure horizons.
        # It is active-protected using recent demand activity and decoder peak evidence to
        # avoid turning real active demand into underforecast when exposure hats are imperfect.
        self.use_exposure_demand_gate = bool(use_exposure_demand_gate)
        self.exposure_gate_threshold = float(exposure_gate_threshold)
        self.exposure_gate_temperature = float(exposure_gate_temperature)
        self.exposure_gate_min = float(exposure_gate_min)
        self.exposure_gate_recent_active_floor = float(exposure_gate_recent_active_floor)
        self.exposure_gate_peak_floor = float(exposure_gate_peak_floor)
    ...
    def _exposure_conditioned_demand_gate(self, future_context, b_t=None, peak_gate=None):
        """Soft one-way constraint: no/low exposure => demand should be near zero.

        Uses external exposure hats from future_context last 3 columns, assumed log1p scale.
        We use max(buy_box_hat_log, in_stock_hat_log) as the exposure evidence.

        Active protection:
          - recent historical demand active weeks provide a floor;
          - decoder peak_gate provides a smaller floor.
        This avoids severe underforecast when the exposure model underpredicts exposure but
        demand history / peak decoder strongly suggests demand.
        """
        B, H, _ = future_context.shape
        if not self.use_exposure_demand_gate:
            return torch.ones(B, H, device=future_context.device, dtype=future_context.dtype)

        stock_log_hat = self._external_exposure_log_hat(future_context)
        # stock_log_hat[...,0]=total, [...,1]=buy_box, [...,2]=in_stock.
        if stock_log_hat.shape[-1] >= 3:
            exposure_signal = torch.maximum(stock_log_hat[:, :, 1], stock_log_hat[:, :, 2])
        else:
            exposure_signal = stock_log_hat.max(dim=-1).values

        temp = max(float(self.exposure_gate_temperature), 1e-4)
        raw_gate = torch.sigmoid((exposure_signal - float(self.exposure_gate_threshold)) / temp)
        gate = raw_gate.clamp(min=float(self.exposure_gate_min), max=1.0)

        # Recent demand activity should prevent a hard collapse if exposure_hat is wrong.
        if b_t is not None:
            recent_active = b_t[:, -min(4, b_t.shape[1]):].float().max(dim=1).values.unsqueeze(1).expand(-1, H)
            recent_floor = float(self.exposure_gate_recent_active_floor) * recent_active
        else:
            recent_floor = torch.zeros_like(gate)

        # Peak decoder confidence also protects potentially active/peak horizons.
        if peak_gate is not None:
            peak_floor = float(self.exposure_gate_peak_floor) * peak_gate.detach().clamp(0.0, 1.0)
        else:
            peak_floor = torch.zeros_like(gate)

        floor = torch.clamp(float(self.exposure_gate_min) + recent_floor + peak_floor, max=0.85)
        return torch.maximum(gate, floor).clamp(0.0, 1.0)
    ...
    def forward(self, x, future_context, nZ=8, *args, **kwargs):
        ...
        for _ in range(nZ):
            ...
            mu_raw = mu_normal + peak_gate * peak_mu
            demand_gate = self._exposure_conditioned_demand_gate(future_context, b_t=b_t, peak_gate=peak_gate)
            mu = mu_raw * demand_gate
...
def run_nb_high_sparse(
    ...
    print("Demand v12d exposure gate: ON | reads exposure_hat_for_demand.csv external hats from current working directory")
