...
def run_exposure_s3_snapshot_rolling(
    ...
    use_encoder_self_attn=True,
    remove_extreme=True,
    output_csv=DEFAULT_FULL_HAT_CSV,
    summary_csv="exposure_snapshot_rolling_summary.csv",
    ...
    Important behavior:
      ...
      2. A snapshot is read, trained, predicted, and released before the next snapshot.
      3. Only exposure hats are concatenated across cuts.
      4. The combined result is saved to `exposure_hat_for_demand full.csv` by default.
      5. Use n_asins=None, max_snapshots=None for all ASINs and all snapshots.
    """
    ...
    for i, row in files.iterrows():
        ...
        try:
            ...
            # IMPORTANT: no SCOT join/intersection.
            result = run_exposure_v2(
                data_raw1=data_cut,
                scot_df=None,
                n_asins=n_asins,
            ...
            # Keep a stable and useful ordering for downstream demand reads.
            sort_cols = [c for c in ["forecast_cut", "asin", "order_week", "horizon"] if c in hat.columns]
            if sort_cols:
    ...
    combined_hats = pd.concat(all_hats, ignore_index=True) if all_hats else pd.DataFrame()
    if combined_hats.empty:
        raise RuntimeError("No snapshot completed successfully; the full hat CSV was not created.")
...
# S3 bucket/prefix are already filled in; no scot_df is needed.
#
# %run -i tcn_exposure_v2_v27_8g_S3_ROLLING_NO_SCOT_FULLHAT.py
#
...
#     patience=2,
#     batch_size=128,
#     output_csv="exposure_hat_for_demand full.csv",
# )
#
